...
def run_full_workflow(
    ...
    collection_name="rag_cookbook_collection",
):
    ...
    if not new_urls:
        ...
        if crawled_count > 0:
            print("=== Step 2: Indexing FDA guidelines into ChromaDB ===")
    ...
    print("\n=== Step 3: Pharmaceutical Marketing Copy Evaluation ===")
...
if __name__ == "__main__":
    urls_path = sys.argv[1] if len(sys.argv) > 1 else "urls.txt"
    output_path = sys.argv[2] if len(sys.argv) > 2 else "output.jsonl"
    seen_path = sys.argv[3] if len(sys.argv) > 3 else "seen_urls.txt"
    collection_name = sys.argv[4] if len(sys.argv) > 4 else "rag_cookbook_collection"
